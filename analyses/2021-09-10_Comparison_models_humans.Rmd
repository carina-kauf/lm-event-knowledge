---
title: "2021-09-10_Comparison_models_humans"
output: html_document
---

NOTE: Currently we standardize the data. We could also do min-max scaling.

NOTE: Synsets seem to only exist for items with ItemNums <= 278!
...
gpt2_df = subset(model_dat, Metric=='gpt2-xl_full')
model_final_sent = gpt2_df$Sentence
model_item_num = gpt2_df$ItemNum
...
model_final_sent = gpt2_df$Sentence
model_item_num = gpt2_df$ItemNum
model_item_plaus = gpt2_df$Plausibility
model_item_voice = gpt2_df$Voice
model_item_synset = gpt2_df$SynonymPair
model_item_numsyn = gpt2_df$NumSyn
#print(paste(model_final_sent[1:5]))
...
dat = rbind(human_dat.mean, model_dat)
```
...
ggplot(data=datAI)+
    geom_violin(mapping = aes(x=factor(Metric, levels=rev(c('human','gpt2-xl_full', 'gpt2_full','bert-large-cased.verb-prob', 'bert-large-cased.sentence-probs-pseudolog', 'bert-large-cased.l2r', 'bert-large-cased.last-word-prob','roberta-large.verb-prob', 'roberta-large.sentence-probs-pseudolog', 'roberta-large.l2r', 'roberta-large.last-word-prob','xlnet-large-cased.verb-prob', 'xlnet-large-cased.sentence-probs-pseudolog', 'xlnet-large-cased.l2r', 'xlnet-large-cased.last-word-prob'))), y=NormScore, fill=Plausibility), 
...
for(i in seq_along(trialtypes)){
  ...
  p <- ggplot(data=curr_data) +
    geom_violin(mapping = aes(x=factor(Metric, levels=rev(c('human','gpt2-xl_full', 'gpt2_full','bert-large-cased.verb-prob', 'bert-large-cased.sentence-probs-pseudolog', 'bert-large-cased.l2r', 'bert-large-cased.last-word-prob','roberta-large.verb-prob', 'roberta-large.sentence-probs-pseudolog', 'roberta-large.l2r', 'roberta-large.last-word-prob','xlnet-large-cased.verb-prob', 'xlnet-large-cased.sentence-probs-pseudolog', 'xlnet-large-cased.l2r', 'xlnet-large-cased.last-word-prob'))), y=NormScore, fill=Plausibility), scale='width', width=0.7, alpha=0.5, position=position_dodge(width=0.6))+
    geom_point(mapping = aes(x=Metric, y=NormScore, fill=Plausibility),
             shape=21, size=1.2, color='black',
             position=position_jitterdodge(jitter.width=0.15, jitter.height=0, dodge.width=.6),
             show.legend = TRUE)+
...
ggsave(paste(savedir,savename,sep="/"), width=12, height=8, units='cm')
```
