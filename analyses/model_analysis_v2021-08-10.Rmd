...
NOTE!
There are 16 extra sentences in new-EventsAdapt_fast_vector_sum.txt!
...
read_data <- function(directory, filename) {
  ...
  if (ncol(d)==3) {
    ...
    d = d  %>%
      ...
      mutate(Metric = str_replace(Metric, "newsentences-EventsAdapt.", "")) %>%
      mutate(Metric = str_replace(Metric, "newsentences_EventsAdapt.", "")) %>%
      #fix names EventsRev
    ...
    if (grepl("deps.prob_sent_", filename, fixed = TRUE) == TRUE){
      d = d %>%
...
# Load data (below yields 28016 (lines)/1648 (sentences) = 17(models)) for EventsAdapt
...
## yields 26368/1648 = 16 > Why is this less now? Didn't use to be the case? And we didn't
#change anything wrt EventsAdapt!
```{r read data, echo=FALSE}
...
### below yields 18128 (lines) /1648 (sentences) = 11 (models) for EventsAdapt
```{r, echo=FALSE}
...
SynonymPair 91 does not have a score for any model for Version2 for some reason!
