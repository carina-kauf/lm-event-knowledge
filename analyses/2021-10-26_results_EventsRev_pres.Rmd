...
savedir <- paste("results_November2021/EventsRevONLY_Models=",which_models)
...
data2plot = dat.binchoice %>% filter(Metric %in% metrics2plot)
```
...
# get p values
dat.binchoice.summary = data2plot %>%
...
plot.binacc = ggplot(data=data2plot, 
...
plot.model2human = ggplot(dat=dat.model2human)+  
  ...
  geom_point(mapping=aes(x=ModelScore, y=HumanScore, color=Category), 
             size=0.6, position=position_jitter(width=0.01))+
...
```{r}
(plot.binacc | plot_spacer() | (plot.model2human / plot.plaus) ) + plot_layout(widths=c(1,0.5, 3))
