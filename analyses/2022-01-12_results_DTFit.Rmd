...
baselines <- c("syntax-based_PPMI","thematicFit_addition","v2_sdm") #"fast_vector_sum", "thematicFit_product",
...
metrics2plot = c(human,ann_group,baseline_group)
...
savedir <- paste("results_Jan2022/DTFit")
...
```
# From now on only plotting ANN data
```{r, echo=FALSE}
# color scheme
color_plaus = '#1b9e77'
color_voice = '#d95f02'
...
df_correlation$Metric = factor(df_correlation$Metric, levels=metrics2plot)
dat$Metric = factor(dat$Metric, levels=metrics2plot)
...
# df_correlation_sub = subset(df_correlation, Metric%in%baselines)
# plot.plaus_baselines = ggplot(data=subset(dat.plaus, Metric%in%baselines))+
#   geom_point(mapping=aes(x=Plausible, y=Implausible), size=0.2, color=color_plaus)+
#   facet_wrap(~Metric, ncol=ncols, nrow=nrows)+
#   geom_abline(slope=1, intercept=0, size=0.2)+
#   geom_point(mapping=aes(x=Plausible, y=Implausible), size=0.2, color=color_plaus)+
...
#   theme_classic()+
#   theme(plot.title = element_text(hjust=0.5,face='bold'))
# plot.plaus_baselines
# 
# savename <- "2_plausibility_scatter_baselines.png"
# ggsave(paste(savedir,savename,sep="/"), width=25, height=9, units='cm')
# ```
## COMBINE

# ```{r}
# 
# plot.plaus_anns / plot.plaus_baselines 
# 
# savename <- "2_plausibility_scatter_all.png"
# ggsave(paste(savedir,savename,sep="/"), height=15,width=28, units='cm')
# ```
...
dat.model2human = dat.model2human %>%
  #add color info
  mutate(Category = ifelse(Metric%in%metrics2plot, "ANNs", "baselines")) 
...
for (t in seq_along(trialTypes)){
  ...
  for(i in seq_along(metrics)){
      ...
      # add vector to a dataframe
      df <- data.frame(current_metric, trialTypes[t], corval, pval2zero, pval2humans)
      df_correlation <- rbind(df_correlation,df)
  }
}
...
# ```{r}
# (plot.binchoice | plot.model2human / plot.plaus_anns) + plot_layout(guides = 'collect', widths = c(1, 2))
# savename <- "composite_test.png"
...
for(i in seq_along(metrics2plot)){
  curr_dat = subset(dat.binchoice,Metric==metrics2plot[i])
