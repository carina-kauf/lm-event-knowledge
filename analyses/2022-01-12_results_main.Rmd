...
baselines <- c("syntax-based_PPMI","thematicFit_addition","v2_sdm") #"order-based_PPMI", "fast_vector_sum", "thematicFit_product",
...
model_dat = model_dat %>%
  mutate(Metric = str_replace(Metric, "gpt2-xl_full-length-normalized", "GPT2-XL-Normalized")) %>%
...
if (normalized_GPT == FALSE){
  ...
  ann_group = c("GPT2-XL","GPT2-XL-Normalized","BERT-large","RoBERTa-large","tinyLSTM")
}
...
plot.plaus.density.sub = ggplot()+
  geom_density(data=dat2plot.sub, mapping=aes(FinalNormScore, fill = factor(Plausibility, levels=c("Implausible","Plausible"))), alpha = 0.2)+
