...
human_dat.comp = human_dat %>%
  ...
  select(WorkerId, ItemNum, Plausibility, Voice, Sentence, FinalNormScore)
...
ggplot(dat=human_dat.sampled)+  
  geom_point(mapping=aes(x=RandomScore, y=MeanScoreRest), size=0.2, 
...
dat.model = dat %>% filter(Metric!="human") %>%
  select(ItemNum, Sentence, Plausibility, FinalNormScore, Metric)
...
val1.human = dat.model2human$ModelScore[dat.model2human$Metric=="singleHuman"]
val2.human = dat.model2human$HumanScore[dat.model2human$Metric=="singleHuman"]

for(i in seq_along(metrics2plot)){
  if (metrics2plot[i]=="human"){
    current_metric="singleHuman"
  } else {
    current_metric=metrics2plot[i]
  }
    val1.model = dat.model2human$ModelScore[dat.model2human$Metric==current_metric]
    val2.model = dat.model2human$HumanScore[dat.model2human$Metric==current_metric]
    ...
    # add vector to a dataframe
    df <- data.frame(current_metric, corvals[2], pval2zero, pval2humans)
    df_correlation <- rbind(df_correlation,df)
}
...
plot.model2human = ggplot(dat=dat.model2human)+  
  geom_point(mapping=aes(x=ModelScore, y=HumanScore), size=0.001, color='gray50',
...
for(i in seq_along(metrics2plot)){
    val1.model = dat.plaus$Plausible[dat.voice$Metric==metrics2plot[i]]
    val2.model = dat.plaus$Implausible[dat.voice$Metric==metrics2plot[i]]
    pval2zero = cor.test(val1.model, val2.model, method="pearson")$p.value
    corvals = c(
      cor(val1.human, val2.human, method="pearson"),
      cor(val1.model, val2.model, method="pearson"),
