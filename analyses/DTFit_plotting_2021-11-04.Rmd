...
chosen_anns = unique(model_dat$Metric)
print(paste(chosen_anns))
```
...
for (i in seq_along(chosen_anns)){
  print(paste(chosen_anns[i]))
  curr_dat = subset(model_dat, Metric==chosen_anns[i])
...
```{r}
for (i in seq_along(chosen_anns)){
  for (j in seq_along(chosen_anns)){
    print(paste(chosen_anns[i],chosen_anns[j]))
    curr_dat1 = subset(model_dat, Metric==chosen_anns[i])
    curr_dat2 = subset(model_dat, Metric==chosen_anns[j])
...
for (i in seq_along(chosen_anns)){
  sub = subset(model_dat, Metric==chosen_anns[i])
  print(paste(chosen_anns[i], "|", length(sub$Sentence), length(unique(sub$Sentence))))
}
...
```{r}
metrics2plot = c(human,chosen_anns)
...
dat.binchoice = dat %>%
  #add color
  mutate(Category = ifelse(Metric%in%chosen_anns, "ANNs", "baselines")) %>%
...
dat.binchoice = dat.binchoice %>%
  mutate(Color = ifelse(Metric=="human","#F8766D", "#00BA38"))
...
#  scale_fill_manual("Color", values=c("#F8766D", "#00BA38"))
...
savename <- "2_plausibility_scatter.png"
ggsave(paste(savedir,savename,sep="/"), width=15, height=12, units='cm')
```
...
#plot.binchoice + plot.model2human + plot.plaus +
#    plot_layout(design = layout)

(plot.binchoice | plot_spacer() | 
   (((plot_spacer() + plot.model2human) + plot_layout(widths=c(1,8))) / plot.plaus) ) + 
