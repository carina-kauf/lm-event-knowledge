...
savedir <- paste("results_November2021/EventsRev=",use_eventsrev,"_Models=",which_models,"_Norm=",normalization,"_baselines")
...
metrics2plot = c(human, baselines)
...
savename <- "model2human_byTrialType_baselines.png"
...
ggsave(paste(savedir,savename,sep="/"), height=12,width=32, units='cm')
#paste("r",round(Correlation,2),sep=" = ")
```
```{r}
print(paste(t.test(c(1,2,3,4,5))))
```
...
for(i in seq_along(metrics2plot)){
  ...
  print(paste(t.test(curr_score_diff)))
  ttest= t.test(curr_score_diff)
  ttestPval2zero = ttest$p.value
  ttestStat = ttest$statistic
  ttestDF = ttest$parameter
  curr_cat = unique(curr_dat$Category)
  print(paste(metrics2plot[i], ttestPval2zero, curr_cat))
  # add vector to a dataframe
    df <- data.frame(metrics2plot[i], ttestPval2zero, ttestStat, ttestDF, curr_cat)
    df_correlation <- rbind(df_correlation,df)
}
...
colnames(df_correlation) = c("Metric", "ttestPval2zero", "ttestStatistic", "DF", "Category", "pLabel")
...
plot.score_difference.aar = ggplot(data=dat.binchoice.aar,mapping=aes(x=TrialType, y=FinalScoreDiff, fill=Category))+
  ...
  #geom_text(mapping=aes(x=1, y=0.015, label=sprintf("t(%1.0f)=%.2f%s", DF, round(ttestStatistic,2), ttestPval2zero)), data=df_correlation, size=cortext_size)+
  ggtitle('SCORE DIFFERENCE')+
  theme_classic()+
  theme(plot.title = element_text(hjust=0.5,face='bold'))+
  xlab("")+
  ylab("Difference (V1 - V2)")+
  coord_cartesian(ylim=c(-0.031,0.015))
...
ggsave(paste(savedir,savename,sep="/"), height=14,width=32, units='cm')
```
...
ggsave(paste(savedir,savename,sep="/"), height=32,width=32, units='cm')
```
...
m0 = lmer(NormScore ~ Plausibility*TrialType*Voice*Metric + agent_freq_norm*Metric + patient_freq_norm*Metric + verb_freq_norm*Metric + sentence_freq_norm*Metric + (1|ItemNum), 
         data=dat, REML=FALSE)
sink("regression_m0.txt", append=FALSE)
...
sink("regression_m1.txt", append=FALSE)
options(max.print=1000000)
summary(m1)
sink()
```
...
m2 = lmer(NormScore ~ Plausibility*TrialType*Voice*Metric + agent_freq_norm*Metric + patient_freq_norm*Metric + verb_freq_norm*Metric + sentence_freq_norm*Metric + (1+Plausibility+Metric|ItemNum), 
         data=dat, REML=FALSE)
sink("regression_m2.txt", append=FALSE)
options(max.print=1000000)
summary(m2)
sink()
```
