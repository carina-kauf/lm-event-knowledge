---
title: "2021-11-24_results_main_pres"
...
savedir <- paste("results_November2021/EventsRev=",use_eventsrev,"_Models=",which_models,"_Norm=",normalization)
...
anns_main <- c("gpt2-xl_full","bert-large-cased.sentence-probs-pseudolog","roberta-large.sentence-probs-pseudolog","xlnet-large-cased.sentence-probs-pseudolog","surprisal_scores_tinylstm")
baselines <- c("syntax-based_PPMI","order-based_PPMI","thematicFit_addition","v2_sdm") #"fast_vector_sum", "thematicFit_product",
...
# exclude baselines and XLNet for plotting purposes (at least for now)
dat.binchoice = dat.binchoice %>%
  filter(Category!="baselines") %>%
  filter(Metric!='xlnet-large-cased')
```
...
# Also exclude XLNet for plotting purposes (at least for now)
...
metrics2plot = metrics2plot[metrics2plot!='xlnet-large-cased']
```
...
levels(dat.model2human$Metric) = c("single human", "GPT2-XL", "BERT-large", "RoBERTa-large", "tinyLSTM")
levels(df_correlation$Metric) = c("single human", "GPT2-XL", "BERT-large", "RoBERTa-large", "tinyLSTM")

# text size 
cortext_size = 3

plot.model2human = ggplot(dat=subset(dat.model2human, TrialType=="AI"))+  
  ...
  geom_point(mapping=aes(x=ModelScore, y=HumanScore, color=Category), 
             ...
             show.legend = FALSE)+
  geom_text(mapping=aes(x=0.2, y=1.15, 
                        label=sprintf("r = %.2f%s", round(Correlation,2), pVal2zeroLabel)), 
            data=subset(df_correlation, TrialType=="AI"), 
            size=cortext_size, hjust = 0)+
...
savename <- "model2human_AI_pres.png"
ggsave(paste(savedir,savename,sep="/"), height=6,width=19, units='cm')


plot.model2human = ggplot(dat=subset(dat.model2human, TrialType=="AAN"))+  
  facet_wrap(~Metric, ncol=ncols)+
  geom_abline(slope=1, intercept=0, size=0.2)+
  geom_point(mapping=aes(x=ModelScore, y=HumanScore, color=Category), 
             size=0.1, position=position_jitter(width=0.01),
  ...
  geom_text(mapping=aes(x=0.2, y=1.15, 
                        label=sprintf("r = %.2f%s", round(Correlation,2), pVal2zeroLabel)), 
            data=subset(df_correlation, TrialType=="AAN"), 
            size=cortext_size, hjust = 0)+
  coord_cartesian(ylim=c(0,1.15), xlim=c(-0.15,1))+
  scale_x_continuous(breaks=c(0,0.5,1))+
  scale_y_continuous(breaks=c(0,0.5,1))+
...
savename <- "model2human_AAN_pres.png"
ggsave(paste(savedir,savename,sep="/"), height=6,width=19, units='cm')


plot.model2human = ggplot(dat=subset(dat.model2human, TrialType=="AAR"))+  
  facet_wrap(~Metric, ncol=ncols)+
  geom_abline(slope=1, intercept=0, size=0.2)+
  geom_point(mapping=aes(x=ModelScore, y=HumanScore, color=Category), 
             size=0.1, position=position_jitter(width=0.01),
  ...
  geom_text(mapping=aes(x=0.2, y=1.15, 
                        label=sprintf("r = %.2f%s", round(Correlation,2), pVal2zeroLabel)), 
            data=subset(df_correlation, TrialType=="AAR"), 
            size=cortext_size, hjust = 0)+
  coord_cartesian(ylim=c(0,1.15), xlim=c(-0.15,1))+
  scale_x_continuous(breaks=c(0,0.5,1))+
  scale_y_continuous(breaks=c(0,0.5,1))+
...
savename <- "model2human_AAR_pres.png"
ggsave(paste(savedir,savename,sep="/"), height=6,width=19, units='cm')
```
...
dat.plaus.sub = dat.plaus %>%
  filter(TrialType%in%c("AI","AAN")) #subsetting here for plausibility 
...
```{r}
# change metric labels
levels(dat.plaus.sub$Metric) = c("single human", "GPT2-XL", "BERT-large", "RoBERTa-large", "tinyLSTM")
levels(df_correlation$Metric) = c("single human", "GPT2-XL", "BERT-large", "RoBERTa-large", "tinyLSTM")
...
plot.plaus = ggplot(data=dat.plaus.sub)+
  ...
  geom_abline(slope=1, intercept=0, size=0.2)+
  geom_point(mapping=aes(x=Plausible, y=Implausible), size=0.2, color=color_plaus)+
  geom_text(mapping=aes(x=0.1, y=1.1, label=sprintf("r = %.2f%s", round(Correlation,2), pVal2zeroLabel)), 
  ...
  scale_y_continuous(breaks=c(0,0.5,1))+
  theme_classic()+
  theme(plot.title = element_text(hjust=0.5,face='bold'))+theme(strip.text.x = element_text(size=10))
plot.plaus

savename <- "plaus_pres.png"
ggsave(paste(savedir,savename,sep="/"), height=6,width=19, units='cm')
```

```{r}
# NOTE: probably need to redo
...
```{r}
# change metric labels
levels(dat.voice$Metric) = c("single human", "GPT2-XL", "BERT-large", "RoBERTa-large", "tinyLSTM")
levels(df_correlation$Metric) = c("single human", "GPT2-XL", "BERT-large", "RoBERTa-large", "tinyLSTM")
...
plot.voice = ggplot(data=dat.voice)+
  ...
  geom_abline(slope=1, intercept=0, size=0.2)+
  geom_point(mapping=aes(x=active, y=passive), size=0.2, color=color_voice)+
  geom_text(mapping=aes(x=0, y=1.1, label=sprintf("r = %.2f%s", round(Correlation,2), pVal2zeroLabel)), 
            data=df_correlation, size=cortext_size, hjust = 0)+
  ggtitle('VOICE')+
  coord_cartesian(ylim=c(0,1.15), xlim=c(-0.15,1))+
  scale_x_continuous(breaks=c(0,0.5,1))+
...
```{r}
# change metric labels
levels(dat.syn$Metric) = c("single human", "GPT2-XL", "BERT-large", "RoBERTa-large", "tinyLSTM")
levels(df_correlation$Metric) = c("single human", "GPT2-XL", "BERT-large", "RoBERTa-large", "tinyLSTM")
