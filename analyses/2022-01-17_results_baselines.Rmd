...
which_models <- "baselines" #can be "anns_all", "anns_main", "baselines", "all", "all_main"
...
human <- c("human")
anns <- c("gpt2","bert","roberta", "tinylstm") #"xlnet"

if (normalized_GPT == FALSE){
  anns_main <- c("gpt2-xl_full","gpt2-xl_full-length-normalized","bert-large-cased.sentence-probs-pseudolog","roberta-large.sentence-probs-pseudolog","surprisal_scores_tinylstm")
} else {
...
baselines <- c("syntax-based_PPMI","thematicFit_addition","v2_sdm", "order-based_PPMI", "thematicFit_product")
baselines = mixedsort(baselines)
...
dat.binchoice = dat %>%
  #add color
  mutate(Category = ifelse(Metric%in%baselines, "baselines", "ANNs")) %>%
...
dat.binchoice$Category = factor(dat.binchoice$Category, levels=c("human", "baselines"))
...
metrics2plot = c(human, c("SDM","ThematicFit","PPMI-syntax"))
...
dat.model2human = dat.model2human %>%
  #add color info
  mutate(Category = ifelse(Metric%in%baseline_group, "baselines", "ANNs")) %>%
  mutate(Category = ifelse(Metric=="singleHuman", "human", Category))

# specify order for plotting
metric_names = c(human,baseline_group)
...
```{r}
# Plotting only humans and ANNs for now - will put baselines in the appendix
metrics2plot = c(human, baseline_group)
...
dat.plaus.sub = dat.plaus %>%
  filter(TrialType%in%c("AI","AAN")) #subsetting here for plausibility as 
dat.plaus.sub$Metric = factor(dat.plaus.sub$Metric, levels=metrics2plot)

# get pairwise correlation values for humans and each model
val1.human = dat.plaus.sub$Plausible[dat.plaus.sub$Metric=="human"]
val2.human = dat.plaus.sub$Implausible[dat.plaus.sub$Metric=="human"]

for(i in seq_along(metrics2plot)){
    val1.model = dat.plaus.sub$Plausible[dat.plaus.sub$Metric==metrics2plot[i]]
    val2.model = dat.plaus.sub$Implausible[dat.plaus.sub$Metric==metrics2plot[i]]
    pval2zero = cor.test(val1.model, val2.model, method="pearson")$p.value
    corvals = c(
      cor(val1.human, val2.human, method="pearson"),
      cor(val1.model, val2.model, method="pearson"),
...
for(i in seq_along(metrics2plot)){
  ...
  curr_data = dat.plaus.sub %>%
    filter(Metric==metrics2plot[i])
  
  curr_df_correlation = df_correlation.plaus %>%
    filter(Metric==metrics2plot[i])
  ...
  geom_point(mapping=aes(x=Plausible, y=Implausible), size=0.2, color=color_plaus)+
  geom_text(mapping=aes(x=0.1, y=1, label=sprintf("r = %.2f%s", round(Correlation,2), pVal2zeroLabel)), 
            data=curr_df_correlation, size=cortext_size, hjust = 0)+
  coord_cartesian(ylim=c(0,1), xlim=c(0,1))+
  ggtitle('PLAUSIBILITY')+
  theme_classic()+
  theme(plot.title = element_text(hjust=0.5,face='bold'))+
  ggtitle(paste(metrics2plot[i]))
  
  p2 <- ggMarginal(p, type="histogram", fill = color_plaus)
  
  assign(nam, p2)
}

grid.arrange(A1, A2, A3, A4,nrow=1)
g <- arrangeGrob(A1, A2, A3, A4)
```
```{r, fig.width=8, fig.height=8}
dat2plot$Metric = factor(dat2plot$Metric, levels=metrics2plot)
dat2plot$TrialType = factor(dat2plot$TrialType, levels=c("AI","AAN","AAR"))
plot.plaus.density = ggplot(dat2plot, aes(FinalNormScore, fill = Plausibility)) + geom_density(alpha = 0.2)+
  facet_grid(Metric~TrialType)+
  #coord_flip()+
  theme_classic()
plot.plaus.density
```
...
dat2plot.sub = dat2plot %>%
  filter(TrialType%in%c("AI","AAN"))

df_correlation.plaus$Metric = factor(df_correlation.plaus$Metric, levels=metrics2plot)
dat2plot.sub$Metric = factor(dat2plot.sub$Metric, levels=metrics2plot)

plot.plaus.density.sub = ggplot()+
  geom_density(data=dat2plot.sub, mapping=aes(FinalNormScore, fill = Plausibility), alpha = 0.2)+
...
```{r}
df_correlation.plaus.aar = data.frame()

dat.plaus.sub = dat.plaus %>%
  filter(TrialType%in%c("AAR")) #subsetting here for plausibility
dat.plaus.sub$Metric = factor(dat.plaus.sub$Metric, levels=metrics2plot)
...
for(i in seq_along(metrics2plot)){
    ...
    # add vector to a dataframe
    df <- data.frame(metrics2plot[i], corvals[2], pval2zero, pval2humans)
    df_correlation.plaus.aar <- rbind(df_correlation.plaus.aar,df)
}
colnames(df_correlation.plaus.aar) = c("Metric", "Correlation", "pVal2zero", "pVal2humans")

# adjust for multiple comparisons (-1 to exclude humans, *3 for 3 metrics we're comparing)
df_correlation.plaus.aar = df_correlation.plaus.aar %>%
  mutate(pVal2zeroAdjusted = p.adjust(pVal2zero, method="fdr", n=length(pVal2zero)*3),
         pVal2humansAdjusted = p.adjust(pVal2humans, method="fdr", n=(length(pVal2humans)-1)*3)) %>%
  mutate(pVal2zeroLabel = ifelse(pVal2zeroAdjusted<0.001, "***", 
                                 ifelse(pVal2zeroAdjusted<0.01, "**",
...
df_correlation.plaus.aar$Metric = factor(df_correlation.plaus.aar$Metric, levels=metrics2plot)
dat.plaus.sub$Metric = factor(dat.plaus.sub$Metric, levels=metrics2plot)
...
plot.plaus.control = ggplot(data=dat.plaus.sub)+
  ...
  geom_text(mapping=aes(x=0, y=1, label=sprintf("r = %.2f%s", round(Correlation,2), pVal2zeroLabel)), 
            data=df_correlation.plaus.aar, size=cortext_size, hjust = 0)+
  coord_cartesian(ylim=c(0,1), xlim=c(0,1))+
  ggtitle('PLAUSIBILITY (BOTH VERSIONS PLAUSIBLE)')+
  theme_classic()+
  xlab("Plausible V1") +
...
df_ttest = data.frame()
for(i in seq_along(metrics2plot)){
  curr_data = subset(dat.plaus.aar.stats, Metric==metrics2plot[i])
  tt = t.test(curr_data$AdjustedScore)
  pval2zero = tt$p.value
  ttstats = tt$statistic
  df <- data.frame(metrics2plot[i], ttstats, pval2zero)
...
<!-- dat.binchoice.aar = subset(dat.binchoice.aar, Metric %in% metrics2plot) -->
<!-- dat.binchoice.aar$Metric = factor(dat.binchoice.aar$Metric, levels=metrics2plot) -->

<!-- df_correlation = data.frame() -->
<!-- # get p values for t test -->
<!-- for(i in seq_along(metrics2plot)){ -->
<!--   curr_dat = subset(dat.binchoice.aar,Metric==metrics2plot[i]) -->
...
<!--   print(paste(metrics2plot[i], ttestPval2zero)) -->
<!--   curr_cat = unique(curr_dat$Category) -->
<!--   # add vector to a dataframe -->
<!--     df <- data.frame(metrics2plot[i], ttestPval2zero,curr_cat) -->
<!--     df_correlation <- rbind(df_correlation,df) -->
<!-- } -->
...
<!-- df_correlation$Metric = factor(df_correlation$Metric, levels=metrics2plot) -->
<!-- dat.binchoice.aar$Metric = factor(dat.binchoice.aar$Metric, levels=metrics2plot) -->
...
dat.voice$Metric = factor(dat.voice$Metric, levels=metrics2plot)
```

### Calculate correlations

```{r}
df_correlation.voice = data.frame()
...
for(i in seq_along(metrics2plot)){
    ...
    # add vector to a dataframe
    df <- data.frame(metrics2plot[i], corvals[2], pval2zero, pval2humans)
    df_correlation.voice <- rbind(df_correlation.voice,df)
}
colnames(df_correlation.voice) = c("Metric", "Correlation", "pVal2zero", "pVal2humans")

# adjust for multiple comparisons (-1 to exclude humans, *3 for 3 metrics we're comparing)
df_correlation.voice = df_correlation.voice %>%
  mutate(pVal2zeroAdjusted = p.adjust(pVal2zero, method="fdr", n=length(pVal2zero)*3),
         pVal2humansAdjusted = p.adjust(pVal2humans, method="fdr", n=(length(pVal2humans)-1)*3)) %>%
  mutate(pVal2zeroLabel = ifelse(pVal2zeroAdjusted<0.001, "***", 
                                 ifelse(pVal2zeroAdjusted<0.01, "**",
...
```{r}
df_correlation.voice$Metric = factor(df_correlation.voice$Metric, levels=metrics2plot)
dat.voice$Metric = factor(dat.voice$Metric, levels=metrics2plot)
...
dat.syn$Metric = factor(dat.syn$Metric, levels=metrics2plot)
```
...
for(i in seq_along(metrics2plot)){
    ...
    # add vector to a dataframe
    df <- data.frame(metrics2plot[i], corvals[2], pval2zero, pval2humans)
    df_correlation.syn <- rbind(df_correlation.syn,df)
}
colnames(df_correlation.syn) = c("Metric", "Correlation", "pVal2zero", "pVal2humans")

# adjust for multiple comparisons (-1 to exclude humans, *3 for 3 metrics we're comparing)
df_correlation.syn = df_correlation.syn %>%
  mutate(pVal2zeroAdjusted = p.adjust(pVal2zero, method="fdr", n=length(pVal2zero)*3),
         pVal2humansAdjusted = p.adjust(pVal2humans, method="fdr", n=(length(pVal2humans)-1)*3)) %>%
  mutate(pVal2zeroLabel = ifelse(pVal2zeroAdjusted<0.001, "***", 
                                 ifelse(pVal2zeroAdjusted<0.01, "**",
...
```{r}
df_correlation.syn$Metric = factor(df_correlation.syn$Metric, levels=metrics2plot)
dat.syn$Metric = factor(dat.syn$Metric, levels=metrics2plot)
...
ggsave(paste(savedir,savename,sep="/"), height=24,width=32, units='cm')
```
...
scatter_dat$TrialType = factor(scatter_dat$TrialType, levels=c("AI", "AAN", "AAR"))

ggplot(data=scatter_dat)+
    geom_point(mapping = aes(x=human, y=gpt2),
             shape=21, size=0.75, color='black',
             show.legend = TRUE)+
  facet_wrap(~TrialType, ncol=ncols, nrow=nrows)+
...
scatter_dat_roberta = subset(dat, Metric=="roberta-large") %>% #best model as per correlation analysis
  select(NormScore) %>%
...
```{r plot}
reg1 <- lm(human~roberta,data=scatter_dat) 
summary(reg1)
	
with(scatter_dat,plot(human, roberta))
abline(reg1)
```
...
m0 = lmer(NormScore ~ Plausibility*TrialType*Voice*Metric + agent_freq_norm*Metric + patient_freq_norm*Metric + verb_freq_norm*Metric + sentence_freq_norm*Metric + (1|ItemNum), 
         data=dat, REML=FALSE)
