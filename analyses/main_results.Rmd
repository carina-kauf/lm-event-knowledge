...
facet_names <- setNames(
  ifelse(!(chosen_models == "human"), paste(chosen_models, "\nprobability", sep=""), paste(chosen_models, "\nplausibility", sep="")),
  chosen_models
)

# Facet names are used to indicate the metric relative to a system (i.e., plausibility vs. probability)
paste(facet_names)
```
...
dat.sub = dat %>%
  ...
  mutate(ScoreType = ifelse(!(Metric == "human"), "probability", "plausibility"))
...
plot.plaus.density.sub = ggplot()+
  ...
  facet_grid(TrialType~Metric, labeller = labeller(TrialType = as_labeller(label_names), Metric = as_labeller(facet_names)))+
...
plot.plaus.density.AI = ggplot()+
  ...
  facet_nested(~ScoreType + Metric) +
  #facet_nested(~TrialType + Metric, labeller = labeller(TrialType = as_labeller(label_names), Metric = as_labeller(facet_names)))+
  geom_vline(xintercept=reference_value, linetype='dotted')+
  theme_classic()+
  ...
  ylab("Density")+
  labs(title = "animate-inanimate, impossible") + 
  theme(plot.title = element_text(hjust=0.5,face='bold'))+
  theme(axis.ticks.y = element_blank(),
        axis.text.y = element_blank(),
        legend.title=element_blank(),
...
plot.plaus.density.AAN = ggplot()+
  ...
  labs(title = "animate-animate, unlikely") + 
  theme(plot.title = element_text(hjust=0.5,face='bold'))+
  theme(axis.ticks.y = element_blank(),
        axis.text.y = element_blank(),
        axis.title = element_text(size=9))
...
dat.plaus = dat.sub %>%
  filter(Metric %in% plotting_models) %>%
  dplyr::select(ItemNum, TrialType, Plausibility, Voice, Metric, FinalNormScore, ScoreType) %>%
  group_by(ItemNum, TrialType, Plausibility, Voice, Metric, ScoreType) %>%
...
df_correlation.plaus.AAN = df_correlation.plaus.AAN %>%
  mutate(ScoreType = ifelse(!(Metric == "human"), "probability", "plausibility"))
df_correlation.plaus.AI = df_correlation.plaus.AI %>%
  mutate(ScoreType = ifelse(!(Metric == "human"), "probability", "plausibility"))
```
...
dat.plaus$Metric = factor(dat.plaus$Metric, levels=plotting_models)
dat.plaus$ScoreType = factor(dat.plaus$ScoreType)
...
plot.plaus.AI = ggplot(data=subset(dat.plaus, TrialType==AI_TrialType))+
  ...
  theme_classic()+
  facet_nested(~ScoreType + Metric) +
  #facet_grid(~Metric, labeller = labeller(Metric = as_labeller(facet_names)))+
