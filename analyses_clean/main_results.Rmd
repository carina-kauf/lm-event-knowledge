...
acc_by_sentence = inner_join(dat.binchoice.human, acc_by_sentence.model %>% select(ItemNum, TrialType, Voice, meanAccuracy)) %>%
  distinct()
...
ggplot(acc_by_sentence, aes(x=meanAccuracy, y=FinalScoreDiff)) + 
  ...
  scale_y_continuous(breaks=c(0,0.5,1))+
  labs(x="# correct model responses", y="Human score difference")+
...
hard_sents = acc_by_sentence %>%
  ...
  select(TrialType, ItemNum, Voice, meanAccuracy, FinalScoreDiff)
...
dat.human.error.sent = subset(dat, Metric=='human') %>% 
  ...
  select(ItemNum,Sentence,Plausibility,Diff) %>%
  distinct() %>%
  spread(Plausibility, Sentence)
