...
# make sure sentences are the same
sentences_shared = intersect(dat_human$Sentence, model_dat$Sentence)
dat = dat %>%
  filter(Sentence %in% sentences_shared)
...
if (any(sapply(dat_dtfit_sentence$Sentence, function(x) {!(x %in% sentences_shared)}))) {
  stop('Not all sentences are in the sentence info file')
}
...
## Save final sentence set
```{r}
sentence_set = dat %>%
  select(ItemNum, Sentence, Plausibility) %>% distinct() 
