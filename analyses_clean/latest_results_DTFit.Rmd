...
dat = do.call(rbind, lapply(files, function(x) read_data_DTFit(dirname_dtfit, x)))
```
...
```{r}
check_for_mismatch <- function(dat_human, model_dat) {
  human_sent = dat_human$Sentence
  message(paste('In model sentence set, but not in human sentence set:'))
  for (i in seq_along(chosen_models)){
    ...
    for (j in seq_along(model_sent)){
      ifelse(model_sent[j]%in%human_sent, "", print(paste(chosen_models[i],'|', model_sent[j])))
    }
  }
  print(paste('*******************'))
  message(paste('In human sentence set, but not in model sentence set:'))
  model_sent = model_dat$Sentence
  for (j in seq_along(human_sent)){
    ifelse(human_sent[j]%in%model_sent, "", print(paste('human |', human_sent[j])))
  }
}

check_for_mismatch(dat_human, model_dat)
```

# Exclude mismatched sentences and their Item partner sentences
```{r}
exclusion_keywords = c('snowball', 
                       'harness')

exclude_sentences <- function(dataframe) {
  message(paste(length(unique(dataframe$Sentence))))
  dataframe = dataframe %>% 
    filter(!grepl(paste(exclusion_keywords, collapse = "|"), Sentence))
  
  sentence_pairnum = dataframe %>%
...
model_dat = exclude_sentences(model_dat)
dat_human = exclude_sentences(dat_human)
```
...
```{r}
# combine all
dat = rbind(dat_human %>% select(ItemNum, Sentence, Score, Typicality, Metric, FinalNormScore, TrialType),
           model_dat %>% select(ItemNum, Sentence, Score, Typicality, Metric, FinalNormScore, TrialType))

chosen_models = c(human, chosen_models)
dat = dat %>% rename(Plausibility=Typicality)
dat$Plausibility = recode(dat$Plausibility, 
                             AT='Implausible', T='Plausible',
                             Atypical='Implausible', Typical='Plausible') 

dat = merge(dat, dat_dtfit_sentence %>% select(-Score), 
...
sentence_set = sentence_set[!duplicated(sentence_set), ]

savedir_sentences <- paste("clean_data")
filename="clean_DTFit_SentenceSet.csv"

write.csv(x=sentence_set, file=paste(savedir_sentences,filename,sep="/"), row.names = FALSE)
```
