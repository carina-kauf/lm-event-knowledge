...
plot.gen.trialtype <- ggplot(data = plot_data.tt,
  ...
  stat_summary(geom='errorbar', fun.data='mean_se',
               size = 0.2, width=0.1)+
  geom_point(data = ceil_data.tt, aes(x=Layer, y=MeanAccuracy, color=TrialType), size=2)+
  geom_errorbar(data = ceil_data.tt, aes(x=Layer, ymin=MeanAccuracy-SE, ymax=MeanAccuracy+SE, color=TrialType), width = 0.1) +
  # geom_text(data = ceil_data.tt, group=ceil_data.tt$TrialType, x =ifelse(ceil_data.tt$Model=="GPT2-xl", ceil_data.tt$Layer - 6, ceil_data.tt$Layer - 3),  y = 0.98,
  #      size = 2.5,
  #      label = "ceiling", 
  #      colour = "#6d6d6d") +
  #theme_classic()+
