...
if (grepl("DTFit_EventsRev_EventsAdapt", experiment)) {
  ...
  label_names_cat <- c(
  "human" = "human",
  "LLMs" = "average LLM model",
  "baselines" = "average baseline model"
  )
...
(p3 <- ggplot(mapping = aes(x = 0:1, y = 1)) +
  theme_void() +
  annotation_custom(img_ai, xmin = 0.05, xmax = .15) +
  annotation_custom(img_ai, xmin = .27, xmax = .37) +
  annotation_custom(img_aa, xmin = .49, xmax = .59) +
  annotation_custom(img_aa, xmin = .7, xmax = .8))
...
ggsave(paste(savedir,savename,sep="/"), width=30, height=7, units='cm', plot.binacc.logos)
```
