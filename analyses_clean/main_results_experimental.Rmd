...
dat.sub$TrialType = factor(dat.sub$TrialType, levels=c("DTFit", "AI", "AAN"))
...
if (experiment=="EventsAdapt") {
  ...
  f = FinalNormScore ~ Plausibility*TrialType*Voice + agent_freq_norm + patient_freq_norm + verb_freq_norm + sentence_freq_norm + NumTokens + (1+Plausibility|ItemNum)
} else {
  datasets = c("DTFit", "EventsRev")
  f = FinalNormScore ~ Plausibility + agent_freq_norm + patient_freq_norm + verb_freq_norm + sentence_freq_norm + NumTokens + (1|ItemNum)
}
```
...
results.clean = results %>% 
  ...
  mutate(Value = ifelse(pValAdjusted<0.05, 
                        ...
                        "")) %>%
  select(Experiment, Metric, Parameter, Value) %>% 
  spread(Metric, Value)
...
results.clean = results.clean %>%
  ...
  select(c(Experiment, Category, Parameter, metrics, 'MeanByModel')) %>%
