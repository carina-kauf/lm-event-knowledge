...
llms_main <- c("RoBERTa-large.PLL.average", "BERT-large.PLL.average", "GPT-J.l2r.average", "GPT2-xl.l2r.average")
...
# shorten model names
llms_main_short <- c("RoBERTa-large", "BERT-large", "GPT-J", "GPT2-xl")
...
convert_metric_names <- function(col) {
  gsub(".surprisal|.prod|.pverb|.l2r.average|.l2r|.PLL.average|.PLL", "", col)
}
...
```
### Binary accuracy plot with active AND passive sentences (for SI)
```{r}
# get p values
...
#### Singular plots for TrialTypes for main figure generation below
...
mid_row <- plot_grid(NULL, plot.plaus.density.AI, NULL, plot.plaus.density.AAN, legend.plaus, label_size = 15, rel_widths = c(0.04, 0.9,0.03, 0.9, 0.2), nrow=1)
bottom_row <- plot_grid(plot.plaus.AI, plot.plaus.AAN, label_size = 15)
bottom_row <- plot_grid(bottom_row, NULL,
                   rel_widths = c(0.9, 0.1),
                   nrow = 1)

title <- ggdraw() + draw_label("PLAUSIBILITY", fontface='bold') + theme(plot.background = element_rect(fill="#F5F5F5", color=NA))
mid_row_title <- plot_grid(title, mid_row, ncol=1, rel_heights=c(0.1, 1)) # rel_heights values control title margins
...
cowplot <- plot_grid(top_row, mid_row_title, bottom_row, labels = c('A', 'B', 'C'), label_size = 15, ncol = 1, rel_heights = c(1.5,0.9,0.9))
cowplot
