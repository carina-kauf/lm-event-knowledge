...
output_path_check_voice_ridge_regression = f'/om2/user/jshe/lm-event-knowledge/probing/parallel_layers/check_sentences_voice_ridge_regression/{model_name}_{dataset_name}_{voice_type}_{sentence_type}.csv'
output_path_check_voice_logit_regression = f'/om2/user/jshe/lm-event-knowledge/probing/parallel_layers/check_sentences_voice_logit_regression/{model_name}_{dataset_name}_{voice_type}_{sentence_type}.csv'
output_path_x_train_x_test_to_r = f'/om2/user/jshe/lm-event-knowledge/probing/parallel_layers/check_sentences_voice_x_train_y_train_to_r/{model_name}_{dataset_name}_{voice_type}_{sentence_type}.csv'
...
for layer in range(layer_num):
  ...
  for reg_trial in range(fold_num):
    ...
    xtrain = np.append(xtrain, x_train)
    ...
    #print('xtrain', x_train[:100], 'ytrain', y_train[:100])
    #logit_model = sm.GLM(y_train, x_train, family=sm.families.Binomial())
    #logit_model = sm.Logit(y_train, x_train)
    #u, s, vt = np.linalg.svd(logit_model.exog, 0)
    #fit = logit_model.fit(maxiter = 50000)
  ...
  check_x_test = np.append(check_x_test, xtest)
  check_x_train = np.append(check_x_train, xtrain)
  check_y_train = np.append(check_y_train, ytrain)

xtrain = pd.DataFrame(check_x_train)
ytrain = pd.DataFrame(check_y_train)
xtest = pd.DataFrame(check_x_test)

xtrain.to_csv(/om2/user/jshe/lm-event-knowledge/probing/parallel_layers/check_sentences_voice_x_train_y_train_to_r/xtrain_bert_active-active)
ytrain.to_csv(/om2/user/jshe/lm-event-knowledge/probing/parallel_layers/check_sentences_voice_x_train_y_train_to_r/ytrain_bert_active-active)
xtest.to_csv(/om2/user/jshe/lm-event-knowledge/probing/parallel_layers/check_sentences_voice_x_train_y_train_to_r/xtest_bert_active-active)
#df_check = pd.DataFrame.from_dict({'Sentence': check_sentences, 'YTest': check_y_test, 'YPred': check_y_pred})
print(df_check)
print(df_check.dtypes)
#df_check.to_csv(output_path_check_voice_logit_regression)
